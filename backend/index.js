...
async function getToken() {
    try {
        const tokenResponse = await axios.get("https://opentdb.com/api_token.php?command=request"); // obtain session key
...
async function generateTrivia(num, token, difficulty) {
    const response = await axios.get(`https://opentdb.com/api.php?amount=${num}&category=22&difficulty=${difficulty}&type=multiple&token=${token}`);
...
async function generateHint(question, correctAnswer, incorrect) {
    let prompt = "Can you generate a hint for this question: " + question + ", with the answer being: " + correctAnswer + " and the wrong answers being: " + incorrect.join(", ");

    const genAI = new GoogleGenerativeAI(process.env.API_KEY);
    const model = genAI.getGenerativeModel({ model: "gemini-1.5-flash", prompt: prompt});
...
app.get('/getcompleted', async (req, res) => {
    try {
        res.status(200).json({ completedDifficulties: req.session.completedDifficulties || []});
    } catch (err) {
        return res.status(500).json({ error: "Internal server error" })
    }
});
...
app.post('/answer', async (req, res) => {
    try {
        ...
        return res.status(200).json({ isCorrect, score: req.session.score, streak: req.session.streak, numCorrect: req.session.numCorrect });
    } catch (err) {
        return res.status(500).json({ error: "Internal server error" });
    }
});
...
app.post('/setgame', async (req, res) => {
    try {
        ...
        res.status(200).json({ numQuestion: req.session.numQuestion, difficulty: req.session.difficulty });
    } catch (err) {
        return res.status(500).json({ error: "Internal server error" })
    }
});
...
app.post('/postleaderboard', async (req, res) => {
    try {
        ...
        return res.status(500).json({ error: "Connecting with database gone wrong." })
    }
})
